...
if __name__ == "__main__":
   ...
   df_meeting_instances.to_csv('./zoom_etl/'+variables['date_time']+'/fact_uuid_last_28_days_'+variables['date_time']+'.csv', index= False)
   ...
   fact_uuid_new.to_csv('./zoom_etl/'+variables['date_time']+'/fact_uuid_new_filter_meeting_ids_'+variables['date_time']+'.csv', index= False)
   ...
   # df_uuid = new_only_fact_uuid.reset_index(drop=True) # Set this for incremental
   # df_uuid = df_uuid.reset_index(drop=True) # Ser this for incremental
   df_uuid = pd.concat([fact_uuid_old, df_uuid_new_only]) # Set this for full download
   ...
   df_uuid.to_csv('./zoom_etl/'+variables['date_time']+'/fact_uuid_etl_status_'+variables['date_time']+'.csv', index= False)
   print('df_master_data shape:', df_master_data.shape)
   ##########################################################################################
   ...
   df_master_data['updated_at'] = variables['date_time']
   df_master_data.to_csv('./zoom_etl/'+variables['date_time']+'/fact_connections_raw_'+variables['date_time']+'.csv', index= False)
   print('Fact_Connections Successful')
   # fact_uuid_old['updated_at'] = fact_uuid_old['end_time'].max()
   df_uuid_new_only['updated_at'] = variables['date_time']
   fact_uuid = pd.concat([fact_uuid_old, df_uuid_new_only])
   fact_uuid.to_csv('./zoom_etl/'+variables['date_time']+'/fact_uuid_master_'+variables['date_time']+'.csv', index= False)
   fact_uuid.to_csv('./Production_Fact_UUID/fact_uuid_master_'+variables['date_time']+'.csv', index= False)
   # fact_uuid.to_csv('fact_uuid_master.csv', index= False)
   fact_uuid_old.to_csv('./zoom_etl/'+variables['date_time']+'/fact_uuid_old_'+variables['date_time']+'.csv', index= False)
   df_uuid_new_only.to_csv('./zoom_etl/'+variables['date_time']+'/fact_uuid_new_'+variables['date_time']+'.csv', index= False)
   ...
   incremental_uniques['updated_at'] = variables['date_time']

   incremental_uniques = incremental_uniques[['user_id', 'location', 'network_type', 'join_time', 'leave_time', 'leave_reason',
                                    'status', 'meeting_id', 'uuid', 'country', 'DID', 'DIDx', 'updated_at']]
   incremental_uniques = incremental_uniques.rename(columns={'location':'user_location', 'status':'user_status'})
   #Save incremental data with unique IDs
   incremental_uniques.to_csv('./zoom_etl/'+variables['date_time']+'/fact_connections_uniques_'+variables['date_time']+'.csv', index= False)
   incremental_uniques.to_csv('./Production_Fact_Connections/fact_connections_uniques_'+variables['date_time']+'.csv', index= False)
   ########################################################################################################
